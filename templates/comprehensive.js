...
async function createComprehensivePDF(data) {
    console.log(data);
  ...
  wrap: true
      ...
      const coverageInfoTable = {
        ...
        datas: [
          ...
          { description: "SUM INSURED:", details: data.additionalData.sum_insured || 'N/A' },
  ...
  wrap: true
      ...
      // Certificate Page
      doc.addPage();
      doc.fontSize(24).text("CERTIFICATE OF INSURANCE", { align: 'center' });
      doc.moveDown(2);

      const certificateInfoTable = {
        ...
        datas: [
          { description: "1. Certificate No:", details: data.additionalData.certificate || 'N/A' },
          { description: "2. Policy No:", details: data.proxyData.policyNo || 'N/A' },
          { description: "3. Index mark and registration number:", details: data.proxyData.vehicleRegNo || 'N/A' },
          {
  description: "4. Make/Model of the Vehicle:",
  details: (data.additionalData.carManufac && data.additionalData.model) ? `${data.additionalData.carManufac} / ${data.additionalData.model}` : 'N/A'
},
          { description: "5. Name of Policy Holder:", details: data.additionalData.name || 'N/A' },
          { description: "6. Effective date of the commencement of insurance:", details: data.additionalData.policy_start_date || 'N/A' },
          { description: "7. Date of expiry of insurance:", details: data.additionalData.policy_expiry_date || 'N/A' },
          { description: "8. Persons or class persons entitled to drive:", details: "The Policyholder and others as permitted." },
          { description: "9. Limitation to Use:", details: "COMPREHENSIVE MOTOR INSURANCE" },
        ],
      };
      doc.table(certificateInfoTable, {
  width: 500,
  prepareHeader: () => doc.font('Times-Bold').fontSize(12),
  prepareRow: (row, i) => doc.font('Times-Roman').fontSize(10),
  columnSpacing: 5,
  ...
  wrap: true
      ...
      // Footer text
      doc.fontSize(9).text(
        "I / We hereby certify that the policy to which this certificate relates is issued in accordance with the provisions of the Motor Vehicles (Third Party Insurance) Ordinance 1945 (Nigeria).",
        { align: 'left' }
      );
      doc.moveDown(1);
...
//      doc.addPage();
//      const fullPageImage1 = 'https://storage.googleapis.com/msgsndr/Y0SAusN39p7rHN1636Df/media/67b339d2f5002630dd5803e9.jpeg';
//      const image2Buffer = Buffer.from(await (await fetch(fullPageImage1)).arrayBuffer());
//      doc.image(image2Buffer, 0, 0, { width: doc.page.width, height: doc.page.height });
//
//      // Second full-page image
//      doc.addPage();
//      const fullPageImage2 = 'https://storage.googleapis.com/msgsndr/Y0SAusN39p7rHN1636Df/media/67b33a944e8a73c6ef42b253.jpeg';
//      const image3Buffer = Buffer.from(await (await fetch(fullPageImage2)).arrayBuffer());
//      doc.image(image3Buffer, 0, 0, { width: doc.page.width, height: doc.page.height });
...
module.exports = createComprehensivePDF;
